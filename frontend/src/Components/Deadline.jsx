...
function Deadline({ task, tasks, setTasks }) {
    ...
    const [inputValue, setInputValue] = useState(() => new Date(task.deadline).toISOString().slice(0, 16))
    ...
    const handleSave = async () => {
        const updatedTasks = tasks.map(t => t.id === task.id ? { ...t, deadline: new Date(inputValue).toISOString() } : t)
